...
img_path = os.path.join(cur_dir,'Original_data/wave/parkinson')
img_list = os.listdir(img_path)



augumented_folder = os.path.join(cur_dir,'wave')
...
for images in img_list:
    ...
    string = 'parkinson_'+images
    name = os.path.splitext(string)[0] 


    cur_img_path = os.path.join('Original_data/wave/parkinson/',images)
    print(name)
    ...
    augumented_train = os.path.join(cur_dir, 'wave/parkinson/')
    print(augumented_train)
    if not os.path.exists(augumented_train):
        os.makedirs(augumented_train)

    # augumented_test = os.path.join(cur_dir, 'spiral/test/parkinson/')
